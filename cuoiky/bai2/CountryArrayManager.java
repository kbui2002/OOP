...
public class CountryArrayManager {
    ...
    private int increment; // Mỗi lần thêm dữ liệu, nếu không đủ chỗ thì cấp phát thêm increment phần tử
                           // cho mảng
    ...
    // Giả sử khi muốn thêm phần tử vào mảng, nhưng mảng đã hết chỗ, ta cần phải cấp
    // phát thêm phần tử cho mảng.
    // Vì mảng có độ dài cố định, nên cần phải cấp phát lại mảng khác có độ dài lớn
    // hơn, sau đó copy dữ liệu từ mảng cũ sang mảng mới.
    ...
    // Vì Manager quản lý mảng chỉ có length phần tử đầu chứa dữ liệu, các phần còn
    // lại là null, nên vị trí thêm vào chính là vị trí có chỉ số length.
    // Trước khi thêm cần phải kiểm tra xem mảng có còn chỗ để thêm hay không.
    // Nếu còn thì thêm vào ở vị trí length, nếu không còn thì phải cấp phát thêm
    // phần tử cho mảng bằng cách gọi hàm allocateMore(),
    ...
    // Trước khi thêm cần phải kiểm tra xem mảng có còn chỗ để thêm hay không.
    // Nếu còn thì thêm dữ liệu vào ở vị trí index, nếu không còn thì phải cấp phát
    // thêm phần tử cho mảng bằng cách gọi hàm allocateMore(),
    // sau đó thêm dữ liệu vào, tăng length lên 1 đơn vị.
    // Vì cấu trúc mảng sau khi được cấp phát là cố định, nên thêm phần tử vào vị
    // trí index có nghĩa là dịch chuyển dữ liệu sau vị trí index sang
    ...
    // Vì cấu trúc mảng sau khi cấp phát là cố định, nên xóa phần tử ở vị trí index
    // có nghĩa là dịch chuyển những dữ liệu sau vị trí index
    // sang trái 1 đơn vị, gán cho dữ liệu ở vị trí length bằng null, độ dài length
    // giảm đi 1 đơn vị.
    ...
    // Vì vậy cấn cấp phát một mảng mới có độ dài là length, copy dữ liệu gốc sang
    // mảng mới,
    // sau đó sắp xếp trên mảng mới, và trả ra ngoài hàm mảng mới đã được sắp xếp.
    // Nếu cần in thông tin theo dân số tăng dần, in thông tin từ mảng dữ liệu mới
    // này.
    // Việc sắp xếp có thể sử dụng các thuật toán sắp xếp, như sellection sort,
    ...
    // Duyện từ đầu mảng đến cuối mảng, xác định xem có bao nhiêu nước thuộc vùng
    // continent,
    // cấp phát một mảng đủ để chứa dữ liệu, sau đó duyệt lại qua mảng để thêm những
    // nước trong vùng continent
    ...
    public CountryData[] filterContinent(String continent) {
        ...
        for (int i = 0; i < this.length; i++) {
            if (this.countryDataArray[i].getContinent().equals(continent)) {
                newArray[count] = this.countryDataArray[i];
                count++;
            }
        }
    ...
    // Có thể làm như sau.
    // Sắp xếp dữ liệu theo thứ tự dân số giảm dần. Cấp phát mảng mới có howMany
    // phần tử,
    ...
    // Hàm lấy danh sách các nước theo code, là String có format dạng, ví dụ [USA
    // VNM].
    public String codeOfCountriesToString(CountryData[] countryDataArray, int length) {
    ...
    // Ví dụ:
    // [CountryData[code='IND', name='India', population=1405888536, area=3287590.0,
    // gdp=1927.7, continent='Asia']
    // CountryData[code='PAK', name='Pakistan', population=229248330, area=881912.0,
    // gdp=1188.9, continent='Asia']]
    // Chú ý chỉ in length phần tử đầu của mảng chứa dữ liệu, các phần tử sau có thể
    // là null.
